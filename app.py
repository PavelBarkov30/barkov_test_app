...
if app_mode == 'Разделение по полу':
    ...
    st.markdown(
        f'''_Требуется проверить гипотезу, что мужчины пропускают в течение года более {work_days} рабочих дней 
        по болезни значимо чаще женщин_''')
    ...
    st.markdown(
        '''Распределения значений в выборках не являются "нормальными", применим непараметрический  
           тест Манна-Уитни для сравнения двух независимых выборок.''')
    ...
    st.markdown(
        f'''_Требуется проверить гипотезу, что Работники старше {age} пропускают в течение года более {work_days} рабочих
        дней  по болезни значимо чаще своих более молодых коллег._''')
    ...
    st.pyplot(fig)
    st.markdown(
        '''Распределения значений в выборках не является "нормальными", применим непараметрический  тест Манна-Уитни 
    ...
    if p > 0.05:
        st.markdown(
            f'''### Вывод: Работники старше  {age} лет не пропускают в течение года более {work_days} рабочих дней 
                    по болезни значимо чаще своих более молодых коллег..''')
    elif p < 0.05:
        st.markdown(
            f'''### Вывод: Работники старше  {age} лет пропускают в течение года более {work_days} рабочих дней
            по болезни значимо чаще своих более молодых коллег..''')
